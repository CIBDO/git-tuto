...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="{{ url('index') }}"><i class="fa fa-dashboard"></i> {{ trans["Catégories d'acte d'imagerie"]}}</a></li>
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            $.ajax({
                url: "{{url('img_items_categories/createImgItemsCategories')}}",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "{{url('img_items_categories/editImgItemsCategories')}}/" + $(this).data('typeid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                        $('body').addClass('loading');
                        $.ajax({
                            url: "{{url('img_items_categories/deleteImgItemsCategories')}}/" + laboCategoriesAnalyse_id,
                            cache: false,
                            async: true
                        })
