...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            {{ trans["Dossier d'analyse médicale"] }}
        </h1>
        <ol class="breadcrumb">
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="dossier_cons">
                    <div class="row">
                      ...
                      {% if (userId == 1) OR in_array("labo_w", userPermissions) OR in_array("labo_a", userPermissions) %}
                        <div class="row">
                          <div class="col-md-12">
...
<script>
    ...
    function actionsFormatter(value, row, index){
      ...
      if(row.etat == "encours"){
        ...
        s += '<a href="{{url("labo_demandes/editDossier2/")}}' + row.patients_id + "/" +row.id + '/2" class="btn btn-xs btn-default ajax-navigation"><i class="fa fa-edit"></i> Résultat</a>&nbsp;&nbsp;';

        s += '<a href="{{url("print/laboDemande/")}}' + row.id + '" class="btn btn-xs btn-default"><i class="fa fa-print"></i> Imprimer</a>&nbsp;&nbsp;';
      ...
      if(row.etat == "création"){
        s += '<a href="{{url("labo_demandes/editDossier2/")}}' + row.patients_id + "/" +row.id + '/1" class="btn btn-xs btn-default ajax-navigation"><i class="fa fa-flask"></i> Prélèvement</a>&nbsp;&nbsp;';
      }
      if(row.etat == "clotûré"){
        s += '<a href="{{url("print/laboDemande/")}}' + row.id + '" class="btn btn-xs btn-default"><i class="fa fa-print"></i> Imprimer</a>&nbsp;&nbsp;';
      }
    ...
    $( document ).ready(function() {
      ...
      $(document).on("click", ".popover-footer .btn" , function(){
        ...
        $('body').on('click', '.createDossier', function () {
            $.ajax({
                url: "{{url('labo_demandes/createDossier')}}/" + $(this).data('patientid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.createDemande', function () {
            ...
            $.ajax({
                url: "{{url('labo_demandes/createDemande')}}/" + $(this).data('patientid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtnAntecedant', function () {
            ...
            swal(
                ...
                .then(function() {
                    $('body').addClass('loading');
                    $.ajax({
                        url: "{{url('labo_demandes/deleteAntecedant')}}/" + id,
                        cache: false,
                        async: true
                    })
