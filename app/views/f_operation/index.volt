...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="{{ url('index') }}"><i class="fa fa-dashboard"></i> {{ trans["Finances"]}}</a></li>
    ...
    <section class="content">
        ...
        <div class="row">
            <div class="col-xs-12" >
            {% if (userId == 1) OR in_array("f_w", userPermissions) OR in_array("f_a", userPermissions) %}
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            ...
            $.ajax({
                url: "{{url('f_operation/createFOperation')}}",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editPopup', function () {
            $('body').addClass('loading');
            $.ajax({
                url: "{{url('f_operation/editFOperation')}}/" + $(this).data('typeassurance'),
                cache: false,
                async: true
            })
            .done(function( html ) {
            ...
            swal(
                    ...
                    function(isConfirm) {
                        if (isConfirm === true) {
                            $('body').addClass('loading');
                            $.ajax({
                                url: "{{url('f_operation/deleteFOperation')}}/" + foperation_id,
                                cache: false,
                                async: true
                            })
