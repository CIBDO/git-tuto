            ...
            <div class="col-md-3">
              ...
              <div class="box box-primary">
                ...
                <div class="box-body">
                  ...
                  <a class="pull-right">{% if sizeof(patient.getImgDemandes()) > 0 %} {{date('d/m/Y H:i:m', strtotime( patient.getImgDemandes(['order' : 'date Desc'])[0].date)) }} {% else %} ------- {% endif %}</a>
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="dossier_cons">
                    <div class="row">
                      ...
                      {% if (userId == 1) OR in_array("img_w", userPermissions) OR in_array("img_a", userPermissions) %}
                        <div class="row">
                          <div class="col-md-12">
...
<script>
    ...
    function actionsFormatter(value, row, index){
      ...
      if(row.etat == "encours"){
        ...
        s += '<a href="{{url("img_demandes/editDossier2/")}}' + row.patients_id + "/" + row.id + '" class="btn btn-xs btn-default ajax-navigation"><i class="fa fa-edit"></i> Résultat</a>&nbsp;&nbsp;';

        s += '<a href="{{url("print/imgDemande/")}}' + row.id + '" class="btn btn-xs btn-default"><i class="fa fa-print"></i> Imprimer</a>&nbsp;&nbsp;';
        
        {% if (userId == 1) OR in_array("img_a", userPermissions) %}
      ...
      if(row.etat == "création"){
        s += '<a href="{{url("img_demandes/editDossier2/")}}' + row.patients_id + "/" +row.id + '" class="btn btn-xs btn-default ajax-navigation"><i class="fa fa-clone"></i> Résultat</a>&nbsp;&nbsp;';
      }
      if(row.etat == "clotûré"){
        s += '<a href="{{url("print/imgDemande/")}}' + row.id + '" class="btn btn-xs btn-default"><i class="fa fa-print"></i> Imprimer</a>&nbsp;&nbsp;';
      }
    ...
    $( document ).ready(function() {
      ...
      $(document).on("click", ".popover-footer .btn" , function(){
        ...
        $('body').on('click', '.createDossier', function () {
            $.ajax({
                url: "{{url('img_demandes/createDossier')}}/" + $(this).data('patientid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.createDemande', function () {
            ...
            $.ajax({
                url: "{{url('img_demandes/createDemande')}}/" + $(this).data('patientid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtnAntecedant', function () {
            ...
            swal(
                ...
                .then(function() {
                    $('body').addClass('loading');
                    $.ajax({
                        url: "{{url('img_demandes/deleteAntecedant')}}/" + id,
                        cache: false,
                        async: true
                    })
        ...
        $('body').on('click', '.clotureBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                        $('body').addClass('loading');
                        $.ajax({
                            url: "{{url('img_demandes/clotureDemande')}}/" + demandeid,
                            cache: false,
                            async: true
                        })
                        .done(function( result ) {
