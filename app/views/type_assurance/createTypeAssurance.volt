<div class="modal-dialog">
    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            {% if form_action == 'edit' %}
                {{ trans['Modifier un organisme assurance'] }}
            {% else %}
                {{ trans['Créer un organisme assurance'] }}
            {% endif %}
            </h4>
        </div>
        {% if form_action == 'edit' %}
        <form action="{{ url('type_assurance/' ~ form_action ~ 'TypeAssurance/' ~ typeAssurance_id) }}" class="form ajaxForm" method="post">
        {% else %}
        <form action="{{ url('type_assurance/' ~ form_action ~ 'TypeAssurance/') }}" class="form ajaxForm" method="post">
        {% endif %}
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="taux" class="control-label">{{ trans['Taux'] }}</label>
                    {{ form.render('taux') }}
                </div>
