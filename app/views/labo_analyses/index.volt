...
<div class="content-wrapper">
    ...
    <section class="content">
         <div class="row">
            <div class="col-xs-12" >
                ...
                <a href="{{ url('labo_analyses/form') }}" class="btn btn-primary pull-right">
                    {{trans["Créer une analyse"]}}
                </a>
    ...
    <div id="createLaboAnalyses" class="modal fade" role="dialog"></div>
    <div id="editLaboAnalyses" class="modal fade" role="dialog"></div>
</div>
...
<script>
    ...
    function actionsFormatter(value, row, index) {
        return '<a href="{{ url("labo_analyses/form") }}/'+ value +'"><i class="glyphicon glyphicon-edit"></i></a>' + 
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "{{url('labo_analyses/editLaboAnalyses')}}/" + $(this).data('antibiotique'),
                cache: false,
                async: true
            })
            .done(function( html ) {
                $('body').removeClass('loading');
                $('#editLaboAnalyses').html(html);
            });
        });
            ...
            swal(
                {
                    ...
                    text: "{{ trans["Cette action supprimera l'analyse nommé:"] }} " + $(this).data('antibiotiquename'),
                    type: 'warning',
                ...
                .then(function() {
                        $('body').addClass('loading');
                        $.ajax({
                            url: "{{url('labo_analyses/deleteLaboAnalyses')}}/" + antibiotique_id,
                            cache: false,
                            async: true
                        })
                        .done(function( result ) {
                            ...
                            if(result == "1"){
                                swal(
                                    '{{ trans["Deleted!"] }}',
                                    "{{ trans["L'analyse a été supprimé avec suucès."] }}",
                                    'success'
                                );
        ...
        columns: [
            ...
            {
                field: 'labo_categories',
                title: "{{ trans['Catégorie'] }}",
                sortable: true,
                filterControl: "input"
            },
            {
