...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="<?= $this->url->get('index') ?>"><i class="fa fa-dashboard"></i> <?= $trans['Produits'] ?></a></li>
    ...
    <section class="content">
                        ...
                        <form class="form-inline" role="form" action="" method="get">
                            ...
                            <div class="row">
                                <div class="form-group  col-md-5" style="margin-right : 10px">
                                    ...
                                    <?= $this->tag->select(['filtre', ['tous' => 'Tous les produits', 'rupture' => 'Produits en rupture de stock', 'seuilMin' => 'Produits en dessous du seuil mininal'], 'useEmpty' => false, 'class' => 'form-control', 'required' => 'required']) ?>
                                </div>

                                <div class="form-group  col-md-4" style="margin-right : 10px">
                                   <!--  <label for="etat"><?= $trans['Etat'] ?> :</label>
                                    <?= $this->tag->select(['etat', ['actif' => 'Actif', 'inactif' => 'Inactif'], 'useEmpty' => false, 'class' => 'form-control', 'required' => 'required']) ?> -->
                                </div>
        ...
        <div class="row">
            <div class="col-xs-12" >
               <?php if (($userId == 1) || in_array('ph_w', $userPermissions) || in_array('ph_a', $userPermissions)) { ?>
...
<script>
    ...
    function libelleFormatter(value, row, index) {
        return '<a title="<?= $trans['Voir tous les details de ce produit'] ?>" href="<?= $this->url->get('produit/fiche') ?>/'+row.id+'">'+value+'</a>';
    }
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            $.ajax({
                url: "<?= $this->url->get('produit/createProduit') ?>",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "<?= $this->url->get('produit/editProduit') ?>/" + $(this).data('produit'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.createCommande', function () {
            ...
            $('body').addClass('loading');
            $.ajax({
                url: "<?= $this->url->get('commande/createCommande') ?>/" + txtID.join(","),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.createReception', function () {
            ...
            $('body').addClass('loading');
            $.ajax({
                url: "<?= $this->url->get('reception/createReception') ?>/0/" + txtID.join(","),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                        $('body').addClass('loading');
                        $.ajax({
                            url: "<?= $this->url->get('produit/deleteProduit') ?>/" + produit_id,
                            cache: false,
                            async: true
                        })
