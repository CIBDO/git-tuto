...
<script>
    ...
    function detailsFormatter(value, row, index) {
        return '<a class="btn btn-primary btn-xs col-xs-12 " href="<?= $this->url->get('commande/details/') ?>'+row.id+'"><i class="fa fa-search pull-left"></i>Details</a>';
    }
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            $.ajax({
                url: "<?= $this->url->get('commande/createCommande') ?>",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                        $('body').addClass('loading');
                        $.ajax({
                            url: "<?= $this->url->get('commande/deleteCommande') ?>/" + commande_id,
                            cache: false,
                            async: true
                        })
