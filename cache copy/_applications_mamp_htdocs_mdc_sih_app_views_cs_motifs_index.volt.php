...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="<?= $this->url->get('index') ?>"><i class="fa fa-dashboard"></i> <?= $trans['Motifs de consultation'] ?></a></li>
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            $.ajax({
                url: "<?= $this->url->get('cs_motifs/createCsMotifs') ?>",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "<?= $this->url->get('cs_motifs/editCsMotifs') ?>/" + $(this).data('csmotifs'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                    $('body').addClass('loading');
                    $.ajax({
                        url: "<?= $this->url->get('cs_motifs/deleteCsMotifs') ?>/" + csmotifsid,
                        cache: false,
                        async: true
                    })
