...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="<?= $this->url->get('index') ?>"><i class="fa fa-dashboard"></i> <?= $trans['Planification'] ?></a></li>
                            ...
                            <div class="row" style="margin-top : 10px">
                                <div class="form-group  col-md-3" style="margin-right : 10px">
                                    <label for="date1"><?= $trans['Année'] ?> :</label>
                                        <?= $this->tag->select(['annee', $annee, 'class' => 'form-control', 'id' => 'annee', 'useEmpty' => true, 'emptyText' => 'Choisir']) ?>
                                </div>
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $('body').on('click', '.createPopup', function () {
            $.ajax({
                url: "<?= $this->url->get('f_planification/createFPlanification') ?>",
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "<?= $this->url->get('f_planification/editFPlanification') ?>/" + $(this).data('fplanification'),
                cache: false,
                async: true
            })
            .done(function( html ) {
            ...
            swal(
                    ...
                    function(isConfirm) {
                        if (isConfirm === true) {
                            $('body').addClass('loading');
                            $.ajax({
                                url: "<?= $this->url->get('f_planification/deleteFPlanification') ?>/" + fplanification_id,
                                cache: false,
                                async: true
                            })
