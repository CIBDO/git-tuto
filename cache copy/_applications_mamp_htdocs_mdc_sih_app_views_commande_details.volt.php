...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="<?= $this->url->get('index') ?>"><i class="fa fa-dashboard"></i> <?= $trans['Commandes'] ?></a></li>
                ...
                <div class="box box-primary">
                    <div class="box-body">
                        <div class="row" style="margin-top : 10px">
                            ...
                            <?php } ?>
                                <a href="<?= $this->url->get('print/detailsCommande/') . $commande['id'] ?>" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box box-info box-solid">
                    ...
                    <div class="box-body">
                        <form action="<?= $this->url->get('commande/detailsAjout') ?>" class="form" method="post">
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box box-info box-solid">
                    ...
                    <div class="box-body">
                        <form action="<?= $this->url->get('commande/details') ?>" class="form" method="post">
                            <div class="table-responsive">
                                <input type="hidden"  name="commande_id" value="<?= $commande['id'] ?>" /> 
                                <table class="table no-margin table-striped">
                                    <thead>
                                    ...
                                    <tbody id="myBody">
                                    <?php foreach ($commandeDetails as $index => $commandeDetail) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number"  min="0" style="width: 70px;" <?php if ($commande['etat'] != 'creation') { ?> readonly="readonly" <?php } ?> name="quantite[]" autocomplete="off" value="<?= $commandeDetail['quantite'] ?>" placeholder="Quantité" />
                                            </td>
                                            <td>
                                                <input type="number" min="0" name="prix[]" style="width: 70px;" <?php if ($commande['etat'] != 'creation') { ?> readonly="readonly" <?php } ?> autocomplete="off" value="<?= $commandeDetail['prix'] ?>" placeholder="Prix" />
                                            </td>
                                            <td>
    ...
    $( document ).ready(function() {
        ...
        function autoloader(){
            $('input.typeahead').typeahead({
                ajax: {
                    url: '<?= $this->url->get('commande/ajaxProduit') ?>/<?= $commande['id'] ?>',
                    method: 'get',
                },
        ...
        $('body').on('click', '.editPopup', function () {
            ...
            $.ajax({
                url: "<?= $this->url->get('commande/editCommande') ?>/" + $(this).data('commandeid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.editReception', function () {
            $('body').addClass('loading');
            $.ajax({
                url: "<?= $this->url->get('reception/createReception') ?>/" + $(this).data('commandeid'),
                cache: false,
                async: true
            })
            .done(function( html ) {
        ...
        $('body').on('click', '.deleteBtn', function () {
            ...
            swal(
                ...
                .then(function() {
                    $('body').addClass('loading');
                    $.ajax({
                        url: "<?= $this->url->get('commande/deleteDetailsItemCommande') ?>/" + itemid,
                        cache: false,
                        async: true
                    })
